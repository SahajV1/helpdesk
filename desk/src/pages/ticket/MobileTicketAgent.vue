...
async function trackTicketOpen() {
  const ticketName = props.ticketId || (route.params.ticketId as string) || "";
  if (!ticketName) return;

  const csrfToken = getCsrfToken();
  const response = await fetch("/api/method/track_ticket_open", {
    method: "POST",
    credentials: "include",
    headers: {
  ...
  if (!response.ok) {
    throw new Error("Failed to track ticket open");
  }
}
