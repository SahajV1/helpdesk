<template>
  <div>
    ...
    <div class="flex min-h-0">
      <aside
        v-if="showMyTicketsViews"
        class="hidden md:block w-56 shrink-0 border-r px-3 py-4"
      >
        <p class="mb-2 text-xs font-semibold tracking-wide text-gray-600">
          {{ __("MY TICKETS") }}
        </p>
        <div class="flex flex-col gap-1">
          <button
            v-for="view in myTicketViews"
            :key="view.key"
            class="w-full rounded-md border px-3 py-2 text-left text-sm"
            :class="
              activeMyTicketView === view.key
                ? 'border-blue-200 bg-blue-50 text-blue-700'
                : 'border-transparent text-gray-700 hover:bg-gray-100'
            "
            @click="applyMyTicketView(view.key)"
          >
            {{ view.label }}
          </button>
        </div>
...
const myTicketViews = [
  { key: "assigned_to_me", label: __("Assigned to Me") },
  { key: "pending", label: __("Pending") },
  { key: "closed_by_me", label: __("Closed by Me") },
];

const showMyTicketsViews = computed(
  () => !isCustomerPortal.value && !isMobileView.value
);
...
type MyTicketViewKey = "assigned_to_me" | "pending" | "closed_by_me";

function buildMyTicketFilters(view: MyTicketViewKey) {
...
function hasMyAssignmentFilter(filters: Record<string, any>) {
  const assignFilter = filters?._assign;
  if (typeof assignFilter === "string") {
    return assignFilter.includes("@me") || assignFilter.includes(userId || "");
  }

  if (Array.isArray(assignFilter)) {
    const values = assignFilter.map((v) => String(v));
    return values.some(
      (value) =>
        value.includes("@me") ||
        value.includes("%@me%") ||
...
function statusFilterKey(statusFilter: any): "pending" | "closed_by_me" | null {
  if (!Array.isArray(statusFilter) || statusFilter.length < 2) return null;

  const operator = String(statusFilter[0]).toLowerCase();
  const values = Array.isArray(statusFilter[1])
    ? statusFilter[1].map((v) => String(v).toLowerCase())
    : String(statusFilter[1])
        .split(",")
        .map((v) => v.trim().toLowerCase());

  const hasResolved = values.includes("resolved");
  const hasClosed = values.includes("closed");

  if (hasResolved && hasClosed && operator === "in") return "closed_by_me";
  if (hasResolved && hasClosed && operator === "not in") return "pending";

  return null;
}

const activeMyTicketView = computed<MyTicketViewKey | null>(() => {
  const filters = (listViewRef.value as any)?.list?.params?.filters || {};
  if (!hasMyAssignmentFilter(filters)) return null;

  if (!filters.status) return "assigned_to_me";
  return statusFilterKey(filters.status);
});

function applyMyTicketView(view: MyTicketViewKey) {
  (listViewRef.value as any)?.applyFilters?.(buildMyTicketFilters(view));
}
